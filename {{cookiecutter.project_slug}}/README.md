...
実験番号を変更するたびに、以下の **2ファイル** を手動で編集してください。
`submit.sh` が実行時に整合性をチェックし、不一致があればエラーで停止します。
...
> - `sub/kernel-metadata.json`: `artifacts/other/001/1` → `artifacts/other/002/1`
...
| `model_sources` | 学習済みモデル | **実験番号変更時に編集** |
...
- [ ] `experiments/{exp}/train.py` を実行してモデルを保存した
...
| `scripts/new_exp.sh [--template TYPE] [--base NUM]` | 新しい実験フォルダを作成（テンプレートまたは過去実験ベース） |
